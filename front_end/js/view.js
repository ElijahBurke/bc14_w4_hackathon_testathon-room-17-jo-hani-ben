...
export async function fetchAndRerender() {
  ...
  if (0 === todos.length) {
    summary.textContent = "😌 No todos found. Looks like you're up-to-date on everything...";
    return;
  }

  const view = createFragmentForTodos(todos);
  ul.append(view);
}
 ...
 * ```
 * const todos = [
 *    { id: 0, task: "Water the plants", completionDate: "2023-05-06" },
 *    { id: 1, task: "Feed the dog", completionDate: "2022-01-01" },
 * ];
 * const fragment = createElementForTodos(todos)
 * ```
 *
 * Note that this function only creates the fragment. It doesn't add it to the page.
 */
function createFragmentForTodos(todos) {
 ...
 * @example
 * ```
 * const todo = { id: 0, task: "Drink water", completionDate: "2023-03-06" };
